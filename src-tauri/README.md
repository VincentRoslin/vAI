...
## Current modules (Phase 11)

| Module | What | Doc |
| ------ | ---- | --- |
| `lib.rs` | The Tauri builder + `run()`; `setup()` loads config + opens/migrates the DB + builds the model registry into managed state; exit hook checkpoints the WAL | — |
...
blob store (with the first blob feature) · `acquisition`
