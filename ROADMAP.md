...
| ---------------- | ------------------------------------------------------- |
| **Phase**        | 21 — Memory                                             |
| **Stage**        | 21.1 — schema + `MemoryRepo` (`IN PROGRESS`)            |
| **Status**       | `IN PROGRESS`                                           |
...
### Phase 21 — Memory *(current pointer)* — `IN PROGRESS` — `docs/plan/21_memory.md`
...
Gate (7): memory extracted + stored after the validation gate; retrieval returns
relevant memories (scripted); memory section within its token budget; retrieval
deterministic for a fixed store+query; survives restart; a memory with prompt
delimiters can't alter prompt structure; scope isolation (per-Persona, FR-56).
