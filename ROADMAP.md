...
| **Stage**        | 22.B (`BLOCKED`) — 22.A `COMPLETE`                      |
| **Status**       | `BLOCKED`                                               |
| **Blocked by**   | owner answers on the 4 phase-entry questions (Krea 2 weights / NF4 cache / LoRA source / venv deps — `docs/plan/22` §Phase-entry questions). 22.B = the real `server.py` + `quantize.py` + `FixedModel::Image` acquisition; 22.C = the RTX 5080 live gate. |
| **Plan doc**     | `docs/plan/22_image-generation.md`                      |
| **Last updated** | 2026-09-06                                              |
| **Updated by**   | phase-22a                                               |
...
### Phase 22 — Image Generation (FLUX.1 Krea) *(current pointer)* — `IN PROGRESS` (22.A `COMPLETE`, 22.B `BLOCKED`) — `docs/plan/22_image-generation.md`
...
`PROPOSED`. **22.B blocked** on owner answers (Krea 2 weights / NF4 cache / LoRA
source / venv deps). Manual LLM evict/restore here — Phase 23 automates it.
...
- **Phase 22.B — image sidecar go-ahead (owner).** 22.A is complete; 22.B (the
  real `image_gen/server.py` + `quantize.py` + `FixedModel::Image` acquisition +
  the ADR-0018 venv gaining `diffusers @ git` / `bitsandbytes` / `peft`) waits on
  the owner's answers to the 4 phase-entry questions in `docs/plan/22`:
  (1) reuse the existing `unsloth/Krea-2-Turbo` HF cache (~34 GB) or re-pull;
  (2) copy the reference project's NF4 quant cache (~9.6 GB) or regenerate here;
  (3) copy the 3 realism LoRAs into `<models.dir>/image/loras/` or download fresh
  (`krea2-lustify-nsfw` is Civitai + a token); (4) OK to add the image deps to
  the one dev `.venv`.
