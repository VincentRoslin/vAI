...
| **Stage**        | 22.A (`IN PROGRESS` — plan finalized; 22.A implementation next) |
| **Status**       | `IN PROGRESS`                                           |
| **Blocked by**   | 22.B/22.C only — need owner answers on the 4 phase-entry questions (Krea 2 weights / NF4 cache / LoRA source / venv deps — `docs/plan/22` §Phase-entry questions). 22.A (blob store + contracts + Rust `image` module vs a fake sidecar) is unblocked. |
| **Plan doc**     | `docs/plan/22_image-generation.md`                      |
| **Last updated** | 2026-09-06                                              |
| **Updated by**   | phase-22-entry                                          |
...
### Phase 22 — Image Generation (FLUX.1 Krea) *(current pointer)* — `IN PROGRESS` (22.A) — `docs/plan/22_image-generation.md`
Local image generation as a resource-managed workload. Owner's sibling-project
sidecar reviewed 2026-09-06 as a reference; plan finalized (stages 22.A/B/C).
Also lands the **content-addressed blob store** (deferred since Phase 9). Manual
LLM evict/restore here — Phase 23 automates it.
