...
| 14 | `14_phase15_llama.md` | Phase 15 — llama.cpp adapter (**split**): `llm/` module (`LlamaBackend`/`LlamaServer`, `server` free-port + `--api-key` bearer + Windows Job Object, `client` `/health` + `/completion` non-stream + SSE stream with deadline + cancel, `protocol` = the only llama.cpp JSON). 13 tests vs an in-process stub. Gate items 4/7/9 + stub sides of 2/3/6 PASS; **1/5/8 + real sides of 2/3/6 NOT EXECUTED — deferred (15.D: no CUDA `llama-server` binary + no real GGUF)** |
