...
| ------ | ----- | ------- |
| `contracts::ids` | `TaskId` · `ModelId` · `ConversationId` · `MessageId` · `PersonaId` · `MemoryId` · `ReservationId` · `WorkerJobId` · `DownloadId` · `AssetId` | Opaque identifiers. Bare non-empty JSON string on the wire; `string` in TS. Deserialization rejects empty / whitespace-only. **ID generation policy (UUID/ULID/rowid) is deferred to Phase 9.** |
...
| voice IPC (Phase 18/19) | `VoiceState` (`Idle`/`Warming`/`Listening`/`Speech`/`Transcribing`/**`Thinking`/`Speaking`/`Interrupting`**/`Error`, tagged `kind`) · `InputDevice` · `OutputDevice` (`name`, `is_default`) | `voice_start(conversation_id, model_id?)` streams `VoiceState` over a Tauri Channel — `model_id` set ⇒ the listen→think→speak loop with barge-in; `voice_input_devices` / `voice_output_devices` list devices. STT worker payload as before; **TTS worker payload** (this phase): request `{ text: string, out_path: string }`, `Ok.data` `{ sample_rate: u32, duration_s: f32 }` — one clause → a WAV the Rust core plays + deletes. `ts-rs`-exported from `voice::`. |
