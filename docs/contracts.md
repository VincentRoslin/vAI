...
| `contracts::model` | `ModelKind` · `ModelBackend` · `Quant` · `ModelCapabilities` · `ModelMetadata` · `ModelState` | Model description + runtime state. **No product model names as string literals** — `display_name` / `backend` are runtime data; logic switches on `ModelKind`. |
