...
| ------ | ----- | ------- |
| `contracts::ids` | `TaskId` · `ModelId` · `ConversationId` · `MessageId` · `PersonaId` · `ReservationId` · `WorkerJobId` · `DownloadId` · `AssetId` | Opaque identifiers. Bare non-empty JSON string on the wire; `string` in TS. Deserialization rejects empty / whitespace-only. **ID generation policy (UUID/ULID/rowid) is deferred to Phase 9.** |
