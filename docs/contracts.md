...
| ------ | ----- | ------- |
| `contracts::ids` | `TaskId` · `ModelId` · `ConversationId` · `MessageId` · `PersonaId` · `MemoryId` · `ReservationId` · `WorkerJobId` · `DownloadId` · `AssetId` · `VoiceId` | Opaque identifiers. Bare non-empty JSON string on the wire; `string` in TS. Deserialization rejects empty / whitespace-only. **ID generation policy (UUID/ULID/rowid) is deferred to Phase 9.** |
...
| `contracts::conversation` | `Role` · `MessageContent` · `GenerationMeta` · `Message` · `ConversationKind` · `Conversation` (`+ persona_id: PersonaId \| null`, Phase 20 — added additively) | One shape for the Persona tab and the Character tab. `MessageContent` is adjacently tagged (`Text` / `Audio` / `Image`). Timestamps are RFC-3339 **strings** — the contract carries no date library. |
| `context::persona` (Phase 20) | `Persona` (`id`, `name`, `summary`, `personality`, `tone`, `style`, `guidance: string[]`, `created_at`, `updated_at`) · `PersonaDraft` (the editable fields) | Structured behaviour data for Tab 1 conversations (FR-30..35). `persona_*` IPC (`persona_list`/`get`/`create`/`update`/`delete`) + `conversation_set_persona` (rejected once a turn exists — FR-17). Field values are **untrusted** — the builder sanitises them. `ts-rs`-exported from `context::`. |
| `context::builder` + `ipc::commands::PromptPreview` (Phase 20) | `PersonaInclusion` (`Absent`/`Full`/`Truncated`) · `Provenance` (`total_tokens`, `budget_tokens`, `system_tokens`, `persona`, `memory_items`, `memory_tokens`, `history_turns_included`, `history_turns_dropped`) · `PromptPreview` (`prompt: string`, `provenance: Provenance`) | `chat_prompt_preview(conversation_id, model_id)` returns the exact assembled prompt + provenance (FR-34). `Provenance` is also logged at `target: "context"` on every generation. |
| `contracts::memory` (Phase 21) | `MemoryKind` (`Fact`/`Preference`/`Event`/`Trait`) · `Memory` (`id`, `kind`, `content`, `importance` 1–5, `source_conversation_id`, `created_at`) | The "view memories" row (FR-52). `memory_list(persona_id)` returns it newest-first; `memory_delete(id)` (FR-53). Content is **untrusted** — sanitised by the builder. `MemoryScope` stays Rust-internal (the wire uses `PersonaId`). `ts-rs`-exported from `contracts::memory`. |
| `contracts::image` (Phase 22, ADR-0006) | `ImageRequest` (`prompt`, `negative?`, `width`, `height`, `steps?`, `guidance?`, `seed?`, `batch_count`, `loras: LoraSelection[]`) + `validate()` · `LoraSelection` (`id`, `weight`) · `ImagePhase` (`Evicting`/`Loading`/`Generating`/`Restoring`) · `ImageProgress` · `GeneratedImageRow` · `ImageEvent` (adjacently tagged: `Progress`/`Done`/`Error`/`Cancelled`) · `ImageLora` · `ImagePreset` · `ImagePresetParams` · ids `ImageLoraId`/`ImagePresetId`/`GeneratedImageId` | Krea 2 Turbo is the only image model — **no `model` field**. `prompt` is **untrusted** (opaque to the sidecar); a LoRA `id` resolves through the registry to a basename-validated file. v1 allows ≤ 1 LoRA (`validate()` rejects more); the list is carried so multi-LoRA is a non-breaking add. `ImageEvent` mirrors `GenerationEvent`. `image_generate`(Channel<ImageEvent>) / `image_cancel` / `image_loras` / `image_presets` / `image_history` / `image_bytes` / `image_output_dir` / `image_open_output_dir` IPC. Every generated PNG is also written to a browsable `<app_data>/images/<stamp>-<seed>.png` alongside the blob store. `ts-rs`-exported from `contracts::image`. |
...
| voice IPC (Phase 18/19) | `VoiceState` (`Idle`/`Warming`/`Listening`/`Speech`/`Transcribing`/**`Thinking`/`Speaking`/`Interrupting`**/`Error`, tagged `kind`) · `InputDevice` · `OutputDevice` (`name`, `is_default`) | `voice_start(conversation_id, model_id?)` streams `VoiceState` over a Tauri Channel — `model_id` set ⇒ the listen→think→speak loop with barge-in; `voice_input_devices` / `voice_output_devices` list devices. STT worker payload as before; **TTS worker payload**: request `{ text: string, out_path: string, voice_wav?: string \| null }`, `Ok.data` `{ sample_rate: u32, duration_s: f32 }` — one clause → a WAV the Rust core plays + deletes. `voice_wav` (Phase 19 follow-up) is an abs reference-WAV path for zero-shot cloning, or `null` for the built-in voice. `ts-rs`-exported from `voice::`. |
| `voice::voices` (Phase 19 follow-up) | `Voice` (`id: VoiceId`, `name`, `active: bool`, `created_at`) | Cloned TTS voices. `voice_list` · `voice_import(name, bytes: number[])` (validated + copied into `<models.dir>/tts/voices/` by Rust) · `voice_delete(id)` · `voice_set_active(id \| null)` (`null` = built-in; applies at the next voice session). `name` is untrusted UI-only text. `ts-rs`-exported from `voice::voices`. |
| diag IPC (Phase 18.5) | `DiagSnapshot` (`build: BuildInfo` · `config`/`models`/`resources`/`lifecycle`: `unknown` · `conversations: ConversationMeta[]` · `recent_logs: string[]` · `host: HostInfo`) | `diag_snapshot` returns it; `diag_export` writes `<app_data>/diagnostics/diag-<ts>.json` and returns the path. **Redacted; no conversation content** — `ConversationMeta` is `{ id, kind, messages, updated_at }` only. `ts-rs`-exported from `diag::`. Local only (ADR-0015). |
